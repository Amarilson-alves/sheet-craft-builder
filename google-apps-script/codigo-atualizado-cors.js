// ============================================================
// CÓDIGO ATUALIZADO COM CORS CORRIGIDO
// ============================================================
...
// TODAS as respostas DEVEM passar por esta função
...
function doGet(e) {
  try {
    ...
    switch (action) {
      case 'getMaterials':
        return corsJson(getMaterials(e));
      case 'searchMaterials':
        return corsJson(searchMaterials(e));
      case 'getObras':
        return corsJson(getObras(e));
      case 'getMaterialsByCategory':
        return corsJson(getMaterialsByCategory(e));
      case 'test':
        return corsJson({ message: 'Conexão bem-sucedida', status: 'OK' });
      default:
        return corsJson({ error: 'Ação não reconhecida' });
    }
  } catch (error) {
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'addMaterial':
        return corsJson(addMaterial(body));
      
      case 'updateMaterial':
        return corsJson(updateMaterial(body));
      
      case 'incrementMaterial':
        return corsJson(incrementMaterial(body));
      
      case 'deleteMaterial':
        return corsJson(deleteMaterial(body));
      
      default:
        return corsJson({ error: 'Ação POST não reconhecida: ' + action });
    }
  } catch (error) {
    return corsJson({ error: error.message, stack: error.stack });
  }
...
function saveObra(e) {
  try {
    ...
    return { success: true, ok: true, message: "Obra salva com sucesso", obraId: obraId };
  } catch (error) {
